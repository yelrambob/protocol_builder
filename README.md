...
- The **main panel** shows exactly one page at a time as a white reading card on the blue background, selected via the sidebar (a small inline script toggles visibility — no page reload). A welcome view with the logo and the book's title (`--book-title`, defaults to "Protocol Book") is shown until something is picked; the same title also sets the browser tab title.
- Each page shows its number, name (or its `title` override, if set — see below), patient type/body part, an optional reference image (from `--protocol-images-base`), exam-level CTDIvol/DLP dose (unless it's a merged pediatric weight-band family — see below), any scanning notes and send-destination from `protocol-overrides.json`, and every series. Scout series get a compact plane/kV/mA table and never show contrast/injection info (scouts are localizers, not diagnostic acquisitions). Other series show any IV contrast volume/injection rate under the series name, then kV/mA (or the min-max range when SmartmA/auto-mA is active) with noise index, pitch, and rotation time per acquisition group, plus a reconstruction table with thickness/interval/kernel/ASIR. ASIR/ASIR-V level is read straight from the scanner's own "AR"+percentage convention (e.g. "AR40" → "40%") — no labels file needed, unlike kernel codes, since it's a fixed GE convention rather than a site-specific code. Derived MPR reformats (coronal/sagittal views reconstructed from an axial series) are shown indented and italicized under their parent reconstruction, inheriting its kernel.
- **Pediatric weight-band families are one page, not one per weight.** GE numbers each pediatric weight variant of the same protocol with a shared third number segment (e.g. "15.7.1"/"15.7.2"/"15.7.3" for a chest protocol banded `<5KG`/`10-20KG`/`ROUTINE`) — the book detects this (any protocol number with 3+ dot-separated segments) and merges the whole family onto a single page under its shared "major.minor" number (e.g. "15.7"), with the weight annotation stripped from the shared name. Exam CTDIvol/DLP is *not* shown as a single line on these pages (it varies per weight); instead a per-weight-band table compares kV, mA, noise index, CTDIvol, and DLP side by side, weight-labeled from whatever's parsed out of each variant's own name (falling back to its number's own trailing segment). Series/reconstruction structure — everything that doesn't vary by weight — is shown once, taken from the lightest variant; a `title`/`notes`/`excluded` override in `protocol-overrides.json` keyed by the shared family number (e.g. `"15.7"`) applies to the whole family at once. A protocol number with only two segments (the usual adult case, or a lone pediatric variant with no sibling weight bands yet) renders exactly as its own single page, unaffected.
